...
//-----------------------------vueからのリクエストに対する処理----------------------------
  @RequestMapping(value = "/reference/{id}", method = RequestMethod.GET) // vue側のgetメソッドと合わせる・・？
  public ResponseEntity<List<Map<String, Object>>> responseReference(@PathVariable("id") Integer id) { // リクエストに応じたメソッド「responseReference」を定義。

    // メソッド① ： child_graphsからデータを取得・整理
    List<ChildReference> childRef = selectChildRef();

    // メソッド② ： parent_graphsからデータを取得・整理
    List<ParentReference> parentRef = selectParentRef(id);
    ...
    // メソッド①〜③までの処理結果をリターン
    return ResponseEntity.ok(arrangeRef);
  }
...
//-------------------①parent-------------------
  private List<ParentReference> selectParentRef(Integer id) {
    final String sql = "select id from parent_graphs where id =:id;";
    SqlParameterSource param = new MapSqlParameterSource().addValue("id", id);
    List<ParentReference> selectParentInfo = jdbcTemplate.query(sql, param, new RowMapper<ParentReference>() {
      public ParentReference mapRow(ResultSet rs, int rowNum) throws SQLException {
        return new ParentReference(rs.getInt("id"));
      }
    });
    return selectParentInfo;
  }
...
// ----------------------------③【①で生成したインスタンスのデータを整形】----------------------------
  ...
  private List<Map<String, Object>> uploadRef(List<ChildReference> childRef, List<ParentReference> parentRef) {
    ...
    for (ParentReference parent : parentRef) {
      ...
      map.put("id", childRef.stream().filter(s -> s.getParent_id() == parent.getId()).collect(Collectors.toList()));
      dataList.add(map);
    }
    return dataList;
  }
